...
void FirstPersonCamera::Roll(float rad)
{
	XMVECTOR right = XMLoadFloat3(&m_Right); // Load Right vector
	XMVECTOR up = XMLoadFloat3(&m_Up);       // Load Up vector
	XMVECTOR look = XMLoadFloat3(&m_Look);   // Load Look (Z-axis) vector

	// Create a rotation matrix to rotate around the Look vector
	XMMATRIX rotationMatrix = XMMatrixRotationAxis(look, rad);

	// World up vector (y-axis)
	XMVECTOR worldUp = XMVectorSet(0.0f, 1.0f, 0.0f, 0.0f);

	// Calculate the current roll angle between the camera Up vector and the world Up vector
	float currentRollCos = XMVectorGetX(XMVector3Dot(right, worldUp));
	float currentRollAngle = asinf(currentRollCos);

	if (fabs(currentRollAngle + rad) > XM_PIDIV4)
		return ;

	// Rotate the Right and Up vectors using the rotation matrix
	right = XMVector3TransformNormal(right, rotationMatrix);
	up = XMVector3TransformNormal(up, rotationMatrix);

	// Store the rotated vectors back to the camera
